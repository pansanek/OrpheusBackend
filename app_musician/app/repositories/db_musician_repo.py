...
from app_musician.app.schemas.base_schema import get_db
...
class MusicianRepo:
    ...
    def update_musician(self, musician: Musician) -> Musician:
        try:
            db_musician = self.db.query(DBMusician).filter(DBMusician.id ==musician.id).first()
            db_musician = musician
            self.db.commit()
            return self.db.query(DBMusician).filter(DBMusician.id ==musician.id).first()
        except Exception as e:
            traceback.print_exc()
            self.db.rollback()
            raise e